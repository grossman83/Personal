...
LSM6DS3::LSM6DS3(uint8_t busType, uint8_t inputArg) : LSM6DS3Core(busType, inputArg) {
    ...
    settings.fifoSampleRate = 400;  //default 10Hz
...
status_t LSM6DS3::begin() {
    ...
    if (settings.accelEnabled == 1) {
        ...
        switch (settings.accelSampleRate) {
            case 0:// turns ODR to 0 Hz (off)
                dataToWrite |= LSM6DS3_ACC_GYRO_ODR_G_POWER_DOWN;
                break;
            case 13:
...
void LSM6DS3::fifoBegin(void) {
    ...
    switch (settings.fifoSampleRate) {
        ...
        case 200:
            ...
            break;
        default:  //** MG ** set default case to 10Hz(slowest)
        case 400:
