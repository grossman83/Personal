...
void loop()
{
  ...
  while(1)
  {
    ...
    // dtostrf(x, 5, 2, xvalstr);  // Convert the float value to a string with 2 decimal places
    // dtostrf(y, 5, 2, yvalstr);
    // dtostrf(z, 5, 2, zvalstr);
    ...
    bleuart.write((uint8_t*)gvalstr, strlen(gvalstr));  // Send the string over BLE
    bleuart.write("\n");


    delay(10);
    for(int i=0; i<25; i++){
      analogWrite(LEDB, i*20);
      delay(2);
    }
    analogWrite(LEDB, 0);
    delay(10);
